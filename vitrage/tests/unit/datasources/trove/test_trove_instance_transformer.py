...
class TroveInstanceTransformerTest(base.BaseTest):
    ...
    def _validate_vertex_props(self, vertex, event):
        self.assertEqual(tb.extract_field_value(event, 'id'),
                         vertex[VProps.ID])

        self.assertEqual(tb.extract_field_value(event, 'name'),
                         vertex[VProps.NAME])

        self.assertEqual(tb.extract_field_value(event, 'status'),
                         vertex[VProps.STATE])

        self.assertEqual(tb.extract_field_value(event, 'tenant_id'),
    ...
    def _validate_server_neighbour(self, neighbour, instance_id, event):
        ...
        server_id = tb.extract_field_value(event, 'server_id')
        self.assertEqual(server_id, vertex[VProps.ID])
